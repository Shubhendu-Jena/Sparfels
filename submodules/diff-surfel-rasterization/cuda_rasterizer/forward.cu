	...
	float* __restrict__ out_others)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Render normal map, squared depth map and color map
			for (int ch = 0; ch < 3; ch++)
			{
				N[ch] += normal[ch] * w;
				color_sq[ch] += features[collected_id[j] * CHANNELS + ch] * features[collected_id[j] * CHANNELS + ch] * w;
			}
			depth_sq += depth * depth * w;
			num_gaussians += 1.0;
	...
	if (inside)
	{
		...
		for (int ch = 0; ch < 3; ch++)
		{
			...
			out_others[pix_id + (COLOR_SQ_OFFSET + ch) * H * W] = color_sq[ch];
		}
		out_others[pix_id + MIDDEPTH_OFFSET * H * W] = median_depth;
		out_others[pix_id + DISTORTION_OFFSET * H * W] = distortion;
		out_others[pix_id + DEPTH_SQ_OFFSET * H * W] = depth_sq;
		out_others[pix_id + NUM_GAUSSIANS_OFFSET * H * W] = num_gaussians;
