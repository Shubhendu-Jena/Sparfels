...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, eval):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        # depth_path = os.path.join(depths_folder, os.path.basename(extr.name)[:-4] + '.npy')
...
def readColmapSceneInfo_base(path, images, eval, llffhold=8):
    try:
    ...
    matches_path = os.path.join(path, "matches/matches.pkl")
    with open(matches_path, 'rb') as f:
        matches_dict = pickle.load(f)

    confidence_path = os.path.join(path, "confidence/conf.pkl")
...
def readColmapSceneInfo_nvs(path, images, eval, args, opt, llffhold=8):
    ...
    if eval and opt.get_video==False:    
        matches_path = os.path.join(path, "init_test_pose", "matches/matches.pkl")
        with open(matches_path, 'rb') as f:
            matches_dict = pickle.load(f)

        confidence_path = os.path.join(path, "init_test_pose", "confidence/conf.pkl")
        with open(confidence_path, 'rb') as f:
            conf_dict = pickle.load(f)
    else:
        matches_path = os.path.join(path, "matches/matches.pkl")
...
def readColmapSceneInfo_dispatch(path, images, eval, args=None, opt=None, llffhold=8):
    use_nvs = bool(opt is not None and getattr(opt, "nvs", False))
    ...
    if use_nvs:
        if opt is None:
            class _Opt:
                get_video = False
            opt = _Opt()
        return readColmapSceneInfo_nvs(path, images, eval, args, opt, llffhold=llffhold)

    return readColmapSceneInfo_base(path, images, eval, llffhold=llffhold)
...
sceneLoadTypeCallbacks = {
    "Colmap": readColmapSceneInfo_dispatch,
