...
def load_images(folder_or_list, size, square_ok=False):
    ...
    for path in folder_content:
        ...
        imgs.append(dict(img=ImgNorm(img)[None], original_img=ImgNorm(original_img)[None], true_shape=np.int32(
            [img.size[::-1]]), idx=len(imgs), instance=str(len(imgs))))
        paths.append(dict(img_path=path))

    assert imgs, 'no images foud at '+root
    print(f' (Found {len(imgs)} images)')
    return imgs, paths, (W1,H1)
...
def save_depth_images(depths, root_path, train_img_list):
    for i, depth in enumerate(depths, 1):  # Starting index at 1
        # breakpoint()
        np.save(os.path.join(root_path, train_img_list[i-1][:-4] + '.npy'), depth)
