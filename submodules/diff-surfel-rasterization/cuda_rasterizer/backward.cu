	...
	float* __restrict__ dL_dcolors)
{
	...
	float dL_ddepth_sq;
	float dL_dcolor_sq[3];

	if (inside) {
		dL_ddepth = dL_depths[DEPTH_OFFSET * H * W + pix_id];
		dL_daccum = dL_depths[ALPHA_OFFSET * H * W + pix_id];
		dL_dreg = dL_depths[DISTORTION_OFFSET * H * W + pix_id];
		dL_ddepth_sq = dL_depths[DEPTH_SQ_OFFSET * H * W + pix_id];
		for (int i = 0; i < 3; i++)
		{
			dL_dnormal2D[i] = dL_depths[(NORMAL_OFFSET + i) * H * W + pix_id];
			dL_dcolor_sq[i] = dL_depths[(COLOR_SQ_OFFSET + i) * H * W + pix_id];
		}
	...
	float last_depth_sq = 0;
	float last_color_sq[3] = {0};

	float accum_depth_rec = 0;
	float accum_alpha_rec = 0;
	float accum_normal_rec[3] = {0};
	float accum_depth_sq_rec = 0;
	float accum_color_sq_rec[3] = {0};
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < 3; ch++) {
				...
				// Propagate gradients to per-Gaussian color squared
				accum_color_sq_rec[ch] = last_alpha * last_color_sq[ch] + (1.f - last_alpha) * accum_color_sq_rec[ch];
				last_color_sq[ch] = collected_colors[ch * BLOCK_SIZE + j] * collected_colors[ch * BLOCK_SIZE + j];
				dL_dalpha += (last_color_sq[ch] - accum_color_sq_rec[ch]) * dL_dcolor_sq[ch];
				// Update also the gradients w.r.t.color of the Gaussian
				atomicAdd(&(dL_dcolors[global_id * C + ch]), 2 * collected_colors[ch * BLOCK_SIZE + j] * dchannel_dcolor * dL_dcolor_sq[ch]);
			}
			// Propagate gradients to per-Gaussian depth squared
			accum_depth_sq_rec = last_alpha * last_depth_sq + (1.f - last_alpha) * accum_depth_sq_rec;
			last_depth_sq = c_d * c_d;
			dL_dalpha += (c_d * c_d - accum_depth_sq_rec) * dL_ddepth_sq;
			...
			// Update the gradients w.r.t. depth of accumulated squared depth
			dL_dz += 2 * alpha * T * c_d * dL_ddepth_sq;
