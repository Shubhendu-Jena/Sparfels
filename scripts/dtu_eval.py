...
parser.add_argument("--skip_training", action="store_false")
parser.add_argument("--skip_rendering", action="store_false")
...
if not args.skip_metrics:
    ...
    for scene in dtu_scenes:
        scan_id = scene[4:]
        ply_file = f"{args.output_path}/{scene}/train/ours_5000/"
        iteration = 30000
        string = f"python {script_dir}/eval_dtu/evaluate_single_scene.py " + \
            f"--input_mesh {args.output_path}/{scene}/train/ours_5000/fuse_post.ply " + \
            f"--scan_id {scan_id} --output_dir output_masked/scan{scan_id} " + \
            f"--mask_dir {args.dtu} " + \
